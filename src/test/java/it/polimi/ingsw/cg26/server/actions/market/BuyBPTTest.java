...
public class BuyBPTTest {
	...
	private Market buildMarket(){
		...
		List<City> bptTobuyCities = new ArrayList<>();
		bptTobuyCities.add(City.createCity("Milano", CityColor.createCityColor("grigio"), new EmptyBonus()));
		bptTobuyCities.add(City.createCity("Roma", CityColor.createCityColor("rosso"), new EmptyBonus()));
		this.bPTToBuy = new BusinessPermissionTile(bptTobuyCities, new EmptyBonus());
		this.bPTToBuy.setOwner(player3);
		this.bPTToBuy.setPrice(9);
		...
		market.addToMarket(this.bPTToBuy);
	...
	public void testBuildActionShouldAssignTheToken(){
		Action action = new BuyBPT(bPTToBuy.getState(), 567);
		
		assertEquals(567, action.getToken());
	}
	...
	public void testApplyActionCheckChanges(){
		Action action = new BuyBPT(this.bPTToBuy.getState(), 1);
		...
		assertEquals(1, gameBoard.getCurrentPlayer().getCoinsNumber());
		assertEquals(9, player3.getCoinsNumber());
